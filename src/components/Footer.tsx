...
export default function Footer({ onSetActiveTab, activeTab }: FooterProps) {
  ...
  return (
    <footer className="bg-zinc-950/80 border-t border-glass relative overflow-hidden" id="page-footer">
      <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-accent-purple via-accent-blue to-accent-teal" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-start pb-8 border-b border-zinc-900">
          ...
          <div className="md:col-span-5 space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-accent-purple to-accent-blue flex items-center justify-center font-display font-black text-white tracking-widest text-sm">
                VG
              </div>
              <div className="flex flex-col">
                <span className="font-display font-extrabold text-xs tracking-tight text-white uppercase">VIBEGROWTH</span>
                <span className="font-mono text-[9px] tracking-widest text-accent-teal uppercase">SOLUTION</span>
              </div>
            </div>
                ...
                <li key={item.id}>
                  <button
                    ...
                    className={`text-xs hover:text-white transition-all cursor-pointer ${
                      activeTab === item.id ? "text-accent-teal font-semibold" : "text-zinc-400 font-light"
                    }`}
          ...
          <div className="md:col-span-4 space-y-4 text-xs">
            ...
            {/* Quick badge */}
            <div className="inline-flex items-center gap-1 bg-white/5 border border-glass px-2.5 py-1 rounded text-[10px] font-mono tracking-tight text-accent-teal">
              <Award size={10} />
        ...
        <div className="pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-[11px] text-zinc-500">
          ...
          <button
            ...
            className="p-2 bg-white/5 hover:bg-white/10 border border-glass text-zinc-400 hover:text-white rounded-xl transition-all flex items-center gap-1 font-mono text-[10px] uppercase cursor-pointer"
